...
export default function StyleSettingsPage() {
  ...
  async function addReference(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const r = await api.post<ReferenceSpeech>("/api/style/references", {
        title,
        content,
      });
      setRefs([r, ...refs]);
  ...
  return (
    <AppShell>
      ...
      <p className="mt-1 text-sm text-neutral-500">
        Your style guide and reference speeches are stuffed into every transformation&apos;s context.
      </p>

      {error && <p className="mt-4 text-sm text-red-600">{error}</p>}

      <form onSubmit={saveGuide} className="mt-6">
        <label className="block text-sm font-medium text-neutral-700">Style guide</label>
        <textarea
          value={guide}
          onChange={(e) => setGuide(e.target.value)}
          rows={10}
          placeholder="Tone, sentence length, rhetorical devices, words to avoid…"
      ...
      <section className="mt-10">
        <h2 className="text-sm font-medium text-neutral-700">Reference speeches</h2>
        <form onSubmit={addReference} className="mt-3 space-y-2">
          ...
          <textarea
            ...
            rows={6}
            placeholder="Paste the speech text"
            required
            className="block w-full rounded-md border border-neutral-300 px-3 py-2 text-sm"
          />
          <button
